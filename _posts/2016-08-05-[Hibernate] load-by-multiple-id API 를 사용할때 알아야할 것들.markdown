...
---
Hibernate 5.1 Release 에 추가된 load-by-multiple-id API 는 여러가지로 유용하게 사용할 수 있다.__
...
2. IN statement를 사용하여 query 를 만들어서 실행한다.
   ...
   > Oracle과 같은 몇개의 database 는 IN statement의 파라미터 갯수에 제한을 둔다.__
   > 많은 엔티티를 한번의 batch 로 조회하게 되면 성능 이슈가 발생할 수 있다.__
   > Hibernate 1차 Cache(Session) 에 이미 올라와 있는 엔티티를 체크하지 않고 모든 엔티티를 database 에서 조회한다.__
